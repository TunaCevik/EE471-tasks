...
def speech_to_text():
    ...
    raw_filename = "temp_stt_raw"
    wav_filename = "temp_stt_input.wav"
    audio_file.save(raw_filename)
    
    try:
        subprocess.run(['ffmpeg', '-y', '-i', raw_filename, '-ar', '16000', '-ac', '1', wav_filename], check=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
